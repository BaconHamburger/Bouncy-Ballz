...
TOP_B = 150 #top edge of window
RIGHT_B = WIN_WIDTH #right edge
LEFT_B = 0 #left edge
BOTTOM_B = WIN_HEIGHT #bottom edge
...
class Ball():
    ...
    def move(self):
        self.rect.move_ip(self.x_dis, self.y_dis) #directions will be a random integer decided when object is created
...
balls = [ball1, ball2, ball3, ball4] #ball object list

player = Player("graphics/green_ball.png") #player object
...
def main():
    ...
    while(True):
        ...
        if game_over == False:
            ...
            clock.tick(FPS) #game loop runs a maximum of 60 times per second
        else:
            window.fill(BACKGROUND_COL)
            player.image = pygame.image.load("graphics/splat.png")
            player.draw(window)
